...
export default function LeaderboardPage() {
  ...
  function getOracle() {
    ...
    const hottest = [...leaderboard].sort(
      (a, b) => b.streak - a.streak
    )[0];

    const coldest = [...leaderboard].sort(
      (a, b) => b.streak - a.streak
    )[0];
    ...
    const maxStreak =
      hottest.streakType === "W" ? hottest.streak : 0;

    const maxLosingStreak =
      coldest.streakType === "L"
        ? coldest.streak
        : 0;
      ...
      {loading ? (
        ...
        <>
          <div className="overflow-hidden rounded-xl border border-gray-200 bg-white">
            <div className="grid grid-cols-[28px_minmax(0,1fr)_32px_32px_48px_72px_58px] items-center border-b border-gray-200 bg-gray-50 px-3 py-3 text-[11px] font-semibold text-gray-500 sm:grid-cols-14 sm:px-6 sm:py-4 sm:text-sm">
            ...
            {leaderboard.map((player, index) => {
              ...
              return (
                <div
                  key={player.user_id}
                  className="grid grid-cols-[28px_minmax(0,1fr)_32px_32px_48px_72px_58px] items-center border-b border-gray-100 px-3 py-3 last:border-b-0 sm:grid-cols-14 sm:px-6 sm:py-4"
                >
                  <div className="font-bold text-gray-400 sm:col-span-1">
                    {index + 1}
                  </div>

                  <div className="min-w-0 font-semibold text-gray-900 sm:col-span-5">
                    <span className="block truncate">
                      {player.user_name}
                    </span>
                  </div>

                  <div className="text-center text-sm font-semibold text-green-600 sm:col-span-2 sm:text-base">
                    {player.wins}
                  </div>

                  <div className="text-center text-sm font-semibold text-red-600 sm:col-span-2 sm:text-base">
                    {player.losses}
                  </div>

                  <div className="text-center text-sm font-semibold text-gray-900 sm:col-span-2 sm:text-base">
                    {winPercentage}%
                  </div>

                  <div
                    className={`text-center text-sm font-bold sm:col-span-2 sm:text-base ${
                      player.net_units > 0
                        ? "text-green-600"
                        : player.net_units < 0
                        ? "text-red-600"
